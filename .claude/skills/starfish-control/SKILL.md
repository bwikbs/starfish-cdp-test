...
| **WebAudio** | disable, enable, getRealtimeData (ack-only stub: no audio engine wired — no real BaseAudioContext / Web Audio graph, enable/disable ack `{}`, getRealtimeData errors `Cannot find BaseAudioContext with such id`, no contextCreated/-Changed/-WillBeDestroyed events) |
| **Media** | disable, enable (ack-only stub: no player registry, enable/disable ack `{}`, no playersCreated/playerPropertiesChanged/playerEventsAdded/playerErrorsRaised events) |
...
| **Debugger** | enable, disable, setBreakpointByUrl, setBreakpoint, getPossibleBreakpoints, getScriptSource, resume, pause, stepOver/Into/Out, continueToLocation, removeBreakpoint, setBreakpointsActive, setSkipAllPauses, setPauseOnExceptions, setAsyncCallStackDepth, setBlackboxPatterns, evaluateOnCallFrame (shape stub: no real pause path — enable returns `{debuggerId:"-1"}`, setBreakpointByUrl returns a `bp-N` id with empty `locations`, getPossibleBreakpoints returns empty `locations`, control/step methods ack `{}`, breakpoints never fire — no Debugger.paused; setBreakpoint errors `Could not resolve breakpoint`, evaluateOnCallFrame errors `Can only perform operation while paused`. EXCEPTION — getScriptSource is REAL: returns `scriptSource` for a script persisted via `Runtime.compileScript {persistScript:true}`, else errors `No script for id <id>`) |
